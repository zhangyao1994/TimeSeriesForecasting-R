...
```{r Update errors for training Comb Model, echo=FALSE}
...
UpdateOneRow <- function(OneRow){
  ...
  if (OneRow$Selection == "TBATS"){
    OneRow$Comb <- OneRow$TBATS
  } else if (OneRow$Selection == "Xgboost") {
    OneRow$Comb <- OneRow$Xgboost
  } else if (OneRow$Selection == "ARIMA") {
    OneRow$Comb <- OneRow$ARIMA
  } else if (OneRow$Selection == "Prophet") {
    OneRow$Comb <- OneRow$Prophet
  } else if (OneRow$Selection == "RF") {
    OneRow$Comb <- OneRow$RF
  } else if (OneRow$Selection == "lm") {
    OneRow$Comb <- OneRow$lm
  }
  return(OneRow$Comb)
}

CombErrors <- apply(MAPE_CV.Table,1,UpdateOneRow)
MAPE_CV.Table$Comb <- CombErrors # APE_FcastRegion_Comb
...
indx <- c('Prophet','ARIMA','TBATS','lm','RF','Xgboost','Comb')
...
```{r Update errors for testing Comb Model, echo=FALSE}
# update each CFG and Region
UpdateOneRow <- function(OneRow){
  if (is.null(unlist(OneRow$Selection))){
