...
```{r Load feather data}
# Load feather data
path <- "CFG_fcast.feather"
# write_feather(CFG_fcast.joined, path)
CFG_fcast.joined <- read_feather(path)
```
...
CFG_fcast <- filter(CFG_fcast,Fiscal_Wk>="FY19W07" & Fiscal_Wk<="FY19W18") %>%
...
Eval.results_fcastRegion <- data.frame(Eval.results_fcastRegion,Region=rep(Region.groups,each = 3),Results=c('AttainmentRates','MAPE','MAPE_median'))
...
```{r Visualize Errors}
ErrorResults4plot <- data.frame(t(Eval.results_fcastRegion))
ErrorResults4plot$Models <- c('MRP','Prophet','ARIMA','TBATS','lm','RF','Xgboost','Comb')
...
rm(data.selected,Eval.results,Eval.results_fcastRegion,Eval.results_wk,results,results.clean)
```
