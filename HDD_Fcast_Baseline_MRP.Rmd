...
for (i in 1:len){
  # HDD Weekly Sales
  temp_data <- filter(CFG_fcast.joined,CFG==CFGgroups[i])
  
  temp_data$HDD_Fcast[temp_data$HDD_Fcast == 0] <- NA
...
MAPE <- lapply(na.omit(results),mean)
MAPE_median <- lapply(na.omit(results),median)
print(MAPE)
print(MAPE_median)
```
