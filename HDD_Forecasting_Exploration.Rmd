---
title: "HDD Forecasting Exploration"
author: "Yao"
date: "May 24-30, 6/4, 2018"
...
f_ets = forecast(m_ets, h=6) # forecast 3 months into the future
plot(f_ets, 
    ylim = c(0, 1000000))
```
...
f_ets = forecast(q_ets, h=forecastPeriodLen) # forecast 3 months into the future
plot(f_ets, 
    ylim = c(0, 2500000))

# ARIMA
q_aa = auto.arima(HDDQuarterlytimeseries)
f_aa = forecast(q_aa, h=forecastPeriodLen)
plot(f_aa, 
    ylim = c(0, 2500000))

# tslm
fit1 <- tslm(HDDQuarterlytimeseries ~ trend + season, lambda=0)
fcast1 <- forecast(fit1,h=forecastPeriodLen)
plot(fcast1, 
    ylim = c(0, 2500000))

# TBATS
m_tbats = tbats(HDDQuarterlytimeseries)
f_tbats = forecast(m_tbats, h=forecastPeriodLen)
plot(f_tbats, 
    ylim = c(0, 2500000))
...
```{r Facebook Prophet Quarterly Forecasting}
HDD_data %>% group_by(Fiscal_Qtr) %>%
  mutate(Fiscal_Qtr_QTY=sum(QTY)) -> HDD_Quarterly

# Convert date format
df <- na.omit(unique(data.frame(ds = as.Date(HDD_Quarterly$Fiscal_Mo_End_Date,format = "%m/%d/%Y"), y = HDD_Quarterly$Fiscal_Qtr_QTY)))

df <- df[!duplicated(df[,c('y')]),]
# ds0 = HDD_Monthly$Fiscal_Mo, used to confirm the Month info

# We call the prophet function to fit the model. The first argument is the historical dataframe. Additional arguments control how Prophet fits the data and are described in later pages of this documentation.
m <- prophet(df)

future <- make_future_dataframe(m, periods = 4, freq = 'quarter')
tail(future)
...
df.cv %>%
  gather(key="key",value="value",y:yhat) %>%
  ggplot(aes(x=ds,y=value,fill=key)) + geom_bar(position="dodge", stat="identity")
