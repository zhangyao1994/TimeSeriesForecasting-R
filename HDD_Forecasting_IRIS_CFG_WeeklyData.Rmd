...
# registerDoParallel(cl)
# getDoParWorkers()
# system.time(foreach(i=1:10000, .export=c('sum', 'tanh')) %dopar% sum(tanh(1:i)))
# 
# 
# registerDoParallel(cl)
# system.time(foreach(i=1:10000) %dopar% sum(tanh(1:i)))
# stopCluster(cl)
# The above tests are done. User's time is saved a little bit.
...
#  .export=c('HDD_Weekly')
Results <- foreach(i_CFG=1:len, .combine=rbind, .packages=c('tidyverse','timetk','tidyquant','broom')) %dopar% {
  # For each CFG
  ...
  if (sum(filter(temp_data,Fiscal_Wk_End_Date>='2017-02-03')$HDD_QTY)>1000){
    ...
    if (nrow(temp_data)>(k+forecastPeriodLen)){
      ...
      for(i in seq(1,(n-k-forecastPeriodLen+1),4)){
        ...
        # Print Errors # Foreach can not print...
