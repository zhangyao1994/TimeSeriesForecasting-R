...
temp_CFG <- temp_qtr %>% spread(Fiscal_Qtr,CFG_Fiscal_Qtr_QTY) %>% t() 

HDD.data <- temp_CFG %>% tail(n=nrow(temp_CFG)-1) %>% data.frame()

HDD.data.numeric <- lapply(HDD.data,function(x) as.numeric(as.character(x))) %>% data.frame()

HDD.data.scaled <- HDD.data.numeric %>% scale() %>% data.frame()
