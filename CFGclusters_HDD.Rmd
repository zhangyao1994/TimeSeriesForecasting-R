...
```{r kmeans cluster}
data.frame(CFG=unique(temp_qtr$CFG),HDD.data.scaled) %>% na.omit() -> HDD.clean
HDD.clean %>% select(starts_with("FY")) %>% kmeans(centers=4, nstart=10) -> km

HDD_clustered <- data.frame(HDD.clean, CFGcluster=factor(km$cluster)) %>% gather(key="Fiscal_Qtr",value="CFG_Fiscal_Qtr_QTY",FY17Q1:FY19Q2)

AllCFG_Qtr.scaled <- HDD_clustered %>% 
...
```{r Dertermine k=4}
HDD_numeric <- HDD.clean %>% select(starts_with("FY")) # make a data table with only the numeric measurements from iris
# calculate the weighted sum squares (wss) for a single cluster
wss <- (nrow(HDD_numeric)-1)*sum(apply(HDD_numeric, 2, var))
# now calculate the weighted sum squares for all cluster numbers from 2 to 15
for (i in 2:15) wss[i] <- sum(kmeans(HDD_numeric,
                                       nstart=10,
                                       centers=i)$withinss)
