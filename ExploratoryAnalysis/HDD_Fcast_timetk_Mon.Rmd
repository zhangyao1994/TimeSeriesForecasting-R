---
title: "HDD Forecasting timetk Monthly"
author: "Yao"
date: "6/4/2018"
...
After testing on weekly data, let me work on Monthly demand here then.
...
# Daiy data
hdd_qty %>% group_by(Fiscal_Mo_End_Date) %>%
  summarise(HDD_QTY=sum(PART_QTY)) -> HDD_Monthly
```

```{r Visualization}
# Select date and count
HDD_Monthly$date <- ymd(HDD_Monthly$Fiscal_Mo_End_Date)

HDD_Monthly <- HDD_Monthly %>%
  filter(date<'2018-06-01') %>% select(date,HDD_QTY)
...
HDD_Monthly %>%
    ...
    geom_line() +
    labs(title = "HDD Monthly Demand: Monthly Scale", x = "Fiscal Date",y='Monthly Demand') +
    theme_tq() +
...
*Two months have large errors and they are around 2016-06 and 2017-06.*
...
Anyways, let me validate this Monthly model.
...
ggplot(aes(x = date), data = HDD_Monthly) +
    labs(title = "HDD Monthly Demand: Monthly Scale", x = "Fiscal Date",y='Monthly Demand') +
    theme_tq() +
    annotate("text", x = ymd("2017-03-20"), y = 220000,
             color = palette_light()[[1]], label = "Train Region") +
...
Let's use our model to predict What are the expected future values for the next six months. The first step is to create the date sequence. Let's use tk_get_timeseries_summary() to review the summary of the dates from the original dataset, "HDD_Monthly".
...
HDD_Monthly %>%
    ...
    labs(title = "HDD Monthly Demand: 6-Month Forecast", x = "Fiscal Date",y='Monthly Demand') +
    theme_tq()
...
HDD_Monthly %>%
    ...
    labs(title = "HDD Monthly Demand: 6-Month Forecast with Prediction Intervals",x = "Fiscal Date",y='Monthly Demand') +
    theme_tq()
```
