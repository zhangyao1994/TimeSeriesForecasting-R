...
6/11/2018 Forecast F19W07-W15 and then compare my forecasting Prophet_fcast to MRP.
...
Prophet_fcast <- foreach(i_CFG=1:len, .combine=rbind, .packages=c('tidyverse','prophet','Rcpp','ggthemes','lubridate','tidyquant')) %dopar% {
  # For each CFG
...
dimnames(Prophet_fcast)[[2]] <- c('CFG',as.character(HDD_Weekly$Fiscal_Wk[112:120]))
Prophet_fcast <- data.frame(Prophet_fcast)
Prophet_fcast[,2:10] <- lapply(Prophet_fcast[,2:10], function(x) as.numeric(as.character(x)))

num_CFG_valid <- nrow(na.omit(Prophet_fcast)) # The total number of CFGs were forecasted.

Prophet_fcast %>% gather(key='date',value='Prophet_fcast',FY19W07:FY19W15) -> Prophet_fcast
saveRDS(Prophet_fcast, 'Prophet_fcast.rds') # my_data <- readRDS(file)
```
