...
xgboost_one = function(data_part_train, data_part_test, one_item){
  ...
  timetk_test_data = mdy(data_part_test_one$ds) %>% 
    ...
    for (cmdy in cmdy_list){
      ...
      if (data_cadence == "Weekly"){
        ...
        cfg_list_tbl_sm_vol = data_part_tmp_train %>% 
          ...
          summarise(y = sum(y)) %>% 
          arrange(desc(y)) %>% 
          mutate(cum_ptg = cumsum(y)/sum(y)) %>% #? What does cumsum(y)/sum(y) tell us?
        ...
        cfg_list_tbl_no_data = data_part_tmp_train %>% 
          ...
          mutate(ds = mdy(Fiscal_Wk_End_Date),
                 rm_flag = (ds + weeks(3) >= max(mdy(data_part_tmp_train$Fiscal_Wk_End_Date)))) %>% 
          filter(rm_flag)
      ...
      result_xgboost = foreach(i = 1:length(cfg_list_test), .combine = rbind, 
                               .packages = c("dplyr", "xgboost", "timetk","lubridate"), 
...
indx <- c('Fiscal_Wk_End_Date','Fiscal_Wk','Fiscal_Mo')
hdd_qty[indx] <- lapply(hdd_qty[indx], function(x) as.character(x))

hdd_qty %>% 
  filter(ESI_Flag=="N" & ITM_TYPE=="HDD") -> hdd_qty

Results_Cinkie <- forecast_cfg(hdd_qty,c("CY17CQ3","CY17CQ4","CY18CQ1","CY18CQ2"),3,'Weekly')

Cinkie_4q <- Results_Cinkie %>%
  group_by(model) %>% 
  summarise(MAPE = mean(ape, na.rm = TRUE),weighted.MAPE = weighted.mean(ape,y, na.rm = TRUE))

Cinkie_q2 <- Results_Cinkie %>%
...
Lock_Guidance_CY18CQ2_Results$model <- 'Lock'
Lock_Guidance_CY18CQ2_Results$CFG <- as.character(Lock_Guidance_CY18CQ2_Results$CFG)

results.joined <- Results_Cinkie %>% 
  filter(test_qtr=='CY18CQ2') %>%
  select(CFG_New,model,ape) %>%
  rename(CFG = CFG_New) %>%
  full_join(Lock_Guidance_CY18CQ2_Results %>% 
              rename(ape = Lock_APE) %>%
              select(CFG,model,ape))
...
results.joined %>%
  ...
  left_join(Lock_Guidance_CY18CQ2_Results %>%
              select(CFG,Actual)) %>%
  left_join(Results_Cinkie %>% 
              filter(test_qtr=='CY18CQ2') %>%
              select(CFG_New,y) %>%
              rename(CFG = CFG_New)) %>%
  unique() -> tmp

models <- colnames(tmp)[2:7]

# Weighted MAPE Calculation
Weighted.MAPEs <- sapply(tmp[models], function(ape) weighted.MAPE = weighted.mean(ape,tmp$y, na.rm = TRUE))
# Normal MAPE Calculation
MAPEs <- sapply(tmp[models], function(ape) MAPE = mean(ape, na.rm = TRUE))
```

```{r Test the functions, eval=FALSE, include=FALSE}
data_part = hdd_qty
...
test_qtr = test_qtrs[1]
cmdy = cmdy_list[1]

one_item = cfg_list_test[1]
```
