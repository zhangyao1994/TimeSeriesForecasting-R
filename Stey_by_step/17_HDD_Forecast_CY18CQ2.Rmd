...
oneCFG_forecast <- function(oneCFG.data,Fcast_Qtrs,MonthAhead = 3, Fcast_Wk_num = 13, forecastPeriodLen = 26){
  ...
  for (Fcast_Qtr in Fcast_Qtrs){
    ...
    temp <-
      train_augmented %>% select(-wday.lbl,-month.lbl,-diff) %>% cbind(ARIMA=head(ARIMA4Xgboost,nrow(train)),Naive=ifelse(nrow(train)>forecastPeriodLen,c(rep(0,forecastPeriodLen),head(train$y,nrow(train)-forecastPeriodLen)),rep(0,nrow(train))))
...
write_feather(All_fcast,"~/Yao_Rdata/All_fcast_CV_Cal_Qtr_0821_ARIMA_Naive.feather")
...
result_4q_yao  %>%
  ...
  gather(Metrics,values,MAPE,weighted.MAPE) %>%
  ungroup() %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive') & Metrics=='MAPE') %>% #'wn','RF','Prophet',
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
...
result_4q_yao  %>%
  ...
  gather(Metrics,values,MAPE,weighted.MAPE) %>%
  ungroup() %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive') & Metrics=='weighted.MAPE') %>% #'wn','RF','Prophet',
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = paste0(round(100*values), '%')), colour = 'white', fontface = 'bold',
...
result_4q_yao  %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive')) %>% #'wn','RF','Prophet',
  select(CFG,Model,AttainmentRate,HDD_Actual,Quarter) %>%
  mutate(Flag = case_when(
    AttainmentRate >= 0.8 & AttainmentRate <= 1.2 ~ 'Green',
    AttainmentRate < 0.8 ~ 'Blue',
...
Accuracy.4q.byQuarter %>%
  ...
  ungroup() %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive', "Lock_Forecast") & Metrics=='weighted.Accuracy') %>% # 'RF','wn','Prophet',
...
All_fcast.wLock_CY18CQ2 <- yao_hdd %>% 
  ...
  full_join(Lock_Guidance_CY18CQ2_Results %>% 
              filter(ITM_TYPE == 'HDD') %>%
...
EvalResults %>%
  ...
  filter(Model %in% c('ARIMA','ets','Prophet','RF','stlar','wn','Xgboost','naive', "Lock_Forecast") & Metrics=='weighted.Accuracy') %>%
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive", "Lock_Forecast"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = paste0(round(100*values), '%')), colour = 'white', fontface = 'bold',
...
EvalResults %>%
  gather(Metrics,values,Accuracy:weighted.MAPE) %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive', "Lock")) %>% # 'wn', "wn",'RF',"RF",'Prophet',  & Metrics != "MAPE"
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "stlar",  "Xgboost", "naive", "Lock"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = paste0(round(100*values), '%')), colour = 'white', fontface = 'bold',
            size = 4, position = position_dodge(width = 0.8), vjust = 1) +
...
```{r Viz selected CFG}
CFG.selected <- 'HDD_4TB_SATA6G_ISE_ESG'
