...
result_yao  %>%
  ...
  filter(Model %in% c('ARIMA','ets','naive','Prophet','RF','stlar','wn','Xgboost')) %>%
...
result_4q_yao  %>%
  ...
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets", "naive","Prophet", "RF", "stlar", "wn", "Xgboost"))) %>%
  filter(!is.na(Model)) %>%
...
MAPE.results %>%
  gather(Metrics,values,MAPE,weighted.MAPE) %>%
  filter(Model %in% c('ARIMA','ets','naive','Prophet','RF','stlar','wn','Xgboost', "Lock_Forecast")) %>%
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets", "naive","Prophet", "RF", "stlar", "wn", "Xgboost", "Lock_Forecast"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = paste0(round(100*values), '%')), colour = 'grey', fontface = 'bold',
            size = 3.5, position = position_dodge(width = 0.8), vjust = 1) +
