...
rename(MRP_Quarterly,"Yao_QTY"="MRP","Quarter"="Calendar_Qtr") -> MRP_Quarterly
...
result_4q_yao_wMRP  %>%
  ...
  gather(Metrics,values,MAPE,weighted.MAPE) %>%
  ungroup() %>%
  filter(Model %in% c("ARIMA", "ets","Prophet", "RF", "stlar", "Xgboost", "naive","MRP") & Metrics=='MAPE') %>% #"wn", 
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive","MRP"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  ...
  theme_bw(base_size = 14) +
  labs(x = '', y = "MAPE",
       title = paste("HDD Quarterly MAPE"),subtitle = paste("Forecast 1 quarter ahead (",length(unique(result_4q_yao$CFG)),'CFGs included )'),fill = "Metrics") +
  facet_wrap(~Quarter) + 
...
result_4q_yao_wMRP %>%
  ...
  gather(Metrics,values,MAPE,weighted.MAPE) %>%
  ungroup() %>%
  filter(Model %in% c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive","MRP") & Metrics=='weighted.MAPE') %>% #'wn','RF','Prophet',
  mutate(Model = ordered(Model, levels = c("ARIMA", "ets","Prophet", "RF", "stlar", "wn", "Xgboost", "naive","MRP"))) %>%
  ggplot(aes(x = Model , y = values, fill = Metrics)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_text(aes(label = paste0(round(100*values), '%')), colour = 'white', fontface = 'bold',
  ...
  labs(x = '', y = "Weighted MAPE",
       title = paste("HDD Quarterly Weighted MAPE"),subtitle = paste("Forecast 1 quarter ahead (",length(unique(result_4q_yao$CFG)),'CFGs included )'),fill = "Metrics") +
  facet_wrap(~Quarter) + 
  theme(legend.position="none")
...
result_4q_yao_wMRP  %>%
 select(CFG,Model ,AttainmentRate,HDD_Actual,Quarter) %>%
  ...
  mutate(Attain_Flag = case_when(
    ...
    AttainmentRate < 0.8~ 'Blue',
    AttainmentRate > 1.2~ 'Red')) %>%
  summarise(Accuracy=length(which(Attain_Flag=="Green"))/length(Attain_Flag),weighted.Accuracy=sum(HDD_Actual[which(Attain_Flag=="Green")])/sum(HDD_Actual)) -> Accuracy.4q.byQuarter
...
result_4q_yao_wMRP  %>%
  ...
  mutate(Flag = coalesce(Flag, "NA")) %>% 
  mutate(Flag = ordered(Flag, levels = c( "NA","Red", "Green", "Blue"))) %>% 
  mutate(Model = ordered(Model, levels = c('ARIMA','ets','stlar','Xgboost','naive','wn','RF','Prophet','MRP'))) %>%
  filter(!is.na(Model)) %>%
  ggplot(aes(x = Model, y = n_cfg_ptg, fill = Flag)) +
  geom_bar(stat = 'identity') +
  geom_text(aes(label = paste0(round(100*n_cfg_ptg), '%')), colour = 'white', fontface = 'bold',
...
Accuracy.4q.byQuarter %>%
  ...
  ungroup() %>%
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive', "Lock_Forecast",'MRP') & Metrics=='weighted.Accuracy') %>% # 'RF','wn','Prophet',
  ...
  labs(x = '', y = "Weighted Accuracy",
       title = paste("HDD Quarterly CFG Weighted Accuracy"), subtitle = paste("Forecast 1 quarter ahead (",length(unique(result_4q_yao$CFG)),'CFGs included )'),fill = "Metrics") + 
  theme(legend.position="none") +
  facet_wrap(~Quarter)
...
#   theme_bw(base_size = 14) +
#   labs(x = '', y = "Errors",
#        title = paste("HDD Quarterly CFG MAPE"), subtitle = paste("CY18CQ2; Forecast 1 quarter ahead (",length(unique(MAPE_q2$CFG)),'CFGs included )'),fill = "Metrics") + 
#   theme(legend.position="bottom")
...
EvalResults %>%
  ...
  theme_bw(base_size = 14) +
  labs(x = '', y = "Weighted Accuracy",
       title = paste("HDD Quarterly CFG Weighted Accuracy"), subtitle = paste("CY18CQ2; Forecast 1 quarter ahead (",length(unique(MAPE_q2$CFG)),'CFGs included )'),fill = "Metrics") + 
  theme(legend.position="none")
...
EvalResults %>%
  ...
  theme_bw(base_size = 14) +
  labs(x = '', y = "Values",
       title = paste("HDD Quarterly Forecast Model Comparison"), subtitle = paste("CY18CQ2; Forecast 1 quarter ahead (",length(unique(MAPE_q2$CFG)),'CFGs included )'),fill = "Metrics") + 
...
result_q2 %>%
  ...
  filter(Model %in% c('ARIMA','ets','stlar','Xgboost','naive', "Lock_Forecast")) %>% #,'Prophet','RF','wn',
