...
results <- foreach(i_CFG=1:len_CFG, .combine=rbind, .packages=c('tidyverse')) %dopar% {
  ...
  for (i_Region in 1:len_Region){
    ...
    weighted_MAPE_MRP <- sum(w*temp_data$err_MRP, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    ...
    RMSE_fcastPeriod_MRP <- abs(sum(temp_data$MRP_Fcast, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_MRP <- mean(temp_data$err_MRP, na.rm = TRUE)
    
    MRP <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'MRP',mape_wk_MRP,WAPE_MRP,APE_fcastPeriod_MRP,weighted_MAPE_MRP,MASE_wk_MRP,AASE_fcastPeriod_MRP,RMSE_wk_MRP,RMSE_fcastPeriod_MRP,MAE_wk_MRP)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_Prophet <- sum(w*temp_data$err_Prophet, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_Prophet <- abs(sum(temp_data$Prophet, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    ...
    Prophet <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'Prophet',mape_wk_Prophet,WAPE_Prophet,APE_fcastPeriod_Prophet,weighted_MAPE_Prophet,MASE_wk_Prophet,AASE_fcastPeriod_Prophet,RMSE_wk_Prophet,RMSE_fcastPeriod_Prophet,MAE_wk_Prophet)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_ARIMA <- sum(w*temp_data$err_ARIMA, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_ARIMA <- abs(sum(temp_data$ARIMA, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_ARIMA <- mean(temp_data$err_ARIMA, na.rm = TRUE)
    
    ARIMA <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'ARIMA',mape_wk_ARIMA,WAPE_ARIMA,APE_fcastPeriod_ARIMA,weighted_MAPE_ARIMA,MASE_wk_ARIMA,AASE_fcastPeriod_ARIMA,RMSE_wk_ARIMA,RMSE_fcastPeriod_ARIMA,MAE_wk_ARIMA)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_TBATS <- sum(w*temp_data$err_TBATS, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_TBATS <- abs(sum(temp_data$TBATS, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_TBATS <- mean(temp_data$err_TBATS, na.rm = TRUE)
    
    TBATS <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'TBATS',mape_wk_TBATS,WAPE_TBATS,APE_fcastPeriod_TBATS,weighted_MAPE_TBATS,MASE_wk_TBATS,AASE_fcastPeriod_TBATS,RMSE_wk_TBATS,RMSE_fcastPeriod_TBATS,MAE_wk_TBATS)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_lm <- sum(w*temp_data$err_lm, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_lm <- abs(sum(temp_data$timetk_lm, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_lm <- mean(temp_data$err_lm, na.rm = TRUE)
    
    lm <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'LinearModel',mape_wk_lm,WAPE_lm,APE_fcastPeriod_lm,weighted_MAPE_lm,MASE_wk_lm,AASE_fcastPeriod_lm,RMSE_wk_lm,RMSE_fcastPeriod_lm,MAE_wk_lm)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_RF <- sum(w*temp_data$err_RF, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_RF <- abs(sum(temp_data$timetk_RF, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_RF <- mean(temp_data$err_RF, na.rm = TRUE)
    
    RF <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'RandomForest',mape_wk_RF,WAPE_RF,APE_fcastPeriod_RF,weighted_MAPE_RF,MASE_wk_RF,AASE_fcastPeriod_RF,RMSE_wk_RF,RMSE_fcastPeriod_RF,MAE_wk_RF)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_Xgboost <- sum(w*temp_data$err_Xgboost, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_Xgboost <- abs(sum(temp_data$timetk_Xgboost, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_Xgboost <- mean(temp_data$err_Xgboost, na.rm = TRUE)
    
    Xgboost <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'Xgboost',mape_wk_Xgboost,WAPE_Xgboost,APE_fcastPeriod_Xgboost,weighted_MAPE_Xgboost,MASE_wk_Xgboost,AASE_fcastPeriod_Xgboost,RMSE_wk_Xgboost,RMSE_fcastPeriod_Xgboost,MAE_wk_Xgboost)
    ...
    # Metric 2: Aggregated Weighted APE
    weighted_MAPE_Ensemble <- sum(w*temp_data$err_Ensemble, na.rm = TRUE)/sum(w*temp_data$HDD_QTY, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Metric 6: Aggregatred RMSE/MAE per week
    RMSE_fcastPeriod_Ensemble <- abs(sum(temp_data$Ensemble, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_Ensemble <- mean(temp_data$err_Ensemble, na.rm = TRUE)
    
    Ensemble <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'Ensemble',mape_wk_Ensemble,WAPE_Ensemble,APE_fcastPeriod_Ensemble,weighted_MAPE_Ensemble,MASE_wk_Ensemble,AASE_fcastPeriod_Ensemble,RMSE_wk_Ensemble,RMSE_fcastPeriod_Ensemble,MAE_wk_Ensemble)
...
dimnames(results)[[2]] <- c('CFG','Region','Model','MAPE_wk','WAPE','APE_fcastPeriod','weighted_MAPE','MASE_wk','AASE_fcastPeriod','RMSE_wk','RMSE_fcastPeriod','MAE_wk')
results <- data.frame(results)
indx <- c('MAPE_wk','WAPE','APE_fcastPeriod','weighted_MAPE','MASE_wk','AASE_fcastPeriod','RMSE_wk','RMSE_fcastPeriod','MAE_wk')
...
path <- "~/Yao_Rdata/MAPE_weighted.feather"
