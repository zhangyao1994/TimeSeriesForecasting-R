...
results <- foreach(i_CFG=1:len_CFG, .combine=rbind, .packages=c('tidyverse')) %dopar% {
  ...
  for (i_Region in 1:len_Region){
    ...
    # Metric 2: Aggregated Weighted MAPE
    ...
    MRP <- c(mape_wk_MRP,WAPE_MRP,APE_fcastPeriod_MRP,weighted_MAPE_MRP,MASE_wk_MRP,AASE_fcastPeriod_MRP,RMSE_wk_MRP,RMSE_fcastPeriod_MRP,MAE_wk_MRP)
    ...
    APE_fcastPeriod_Prophet <- abs(sum(temp_data$Prophet)-totalHDD)/totalHDD * 100
    
    # ARIMA
    # Weekly Error Calculation
    mape_wk_ARIMA<- weighted.mean(temp_data$ape_ARIMA,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_ARIMA <- abs(sum(temp_data$ARIMA)-totalHDD)/totalHDD * 100
    
    # TBATS
    # Weekly Error Calculation
    mape_wk_TBATS<- weighted.mean(temp_data$ape_TBATS,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_TBATS <- abs(sum(temp_data$TBATS)-totalHDD)/totalHDD * 100
    
    # timetk lm
    # Weekly Error Calculation
    mape_wk_lm<- weighted.mean(temp_data$ape_lm,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_lm <- abs(sum(temp_data$timetk_lm)-totalHDD)/totalHDD * 100
    
    # timetk RF
    # Weekly Error Calculation
    mape_wk_RF<- weighted.mean(temp_data$ape_RF,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_RF <- abs(sum(temp_data$timetk_RF)-totalHDD)/totalHDD * 100
    
    # timetk Xgboost
    # Weekly Error Calculation
    mape_wk_Xgboost<- weighted.mean(temp_data$ape_Xgboost,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_Xgboost <- abs(sum(temp_data$timetk_Xgboost)-totalHDD)/totalHDD * 100
    
    # Ensemble
    # Weekly Error Calculation
    mape_wk_Ensemble<- weighted.mean(temp_data$ape_Ensemble,temp_data$HDD_QTY, na.rm = TRUE)
    # 12-week Error Calculation
    APE_fcastPeriod_Ensemble <- abs(sum(temp_data$Ensemble)-totalHDD)/totalHDD * 100
