...
results4AR %>% group_by(Model) %>% left_join(TotalHDD_FY18) %>%
  summarize(WeightedAttainmentRate_AAPE=sum(ifelse(APE_fcastPeriod<=20,1,0)*weight*100),
            WeightedAttainmentRate_AWAPE=sum(ifelse(Aggregated_weighted_APE<=20,1,0)*weight*100),
            WeightedAttainmentRate_MAPE_wk=sum(ifelse(MAPE_wk<=20,1,0)*weight*100),
            WeightedAttainmentRate_WAPE=sum(ifelse(WAPE<=20,1,0)*weight*100)) -> WeightedAttainmentRate.results
