...
myforecast <- function(temp_data){
  ...
  if (nrow(test)<1 | nrow(train)<nrow(test)){
    return(c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,rep(NA,forecastPeriodLen+1)))
  }
  ...
  Naive <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'naive',tail(train$y,forecastPeriodLen))
  ...
  return(rbind(Truth,Naive,TBATS_fcast,ARIMA_fcast,Prophet_fcast,lm_fcast,RF_fcast,Xgboost_fcast,ets_fcast,rw_fcast,rwd_fcast,wn_fcast,nn_fcast)) # stlar_fcast does not work
}
```
