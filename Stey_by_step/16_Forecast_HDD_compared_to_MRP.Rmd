...
week_data.2 <- hdd_qty_updated0731 %>% group_by(Fiscal_Wk) %>%
...
The difference seems acceptable and not huge.
...
myforecast <- function(temp_data){
  ...
  Truth <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'Actual',test$y,rep(NA,forecastPeriodLen-AvailableWeek))
  
  if (nrow(test)<=1 | nrow(train)<=nrow(test) | nrow(train)<=2){
    return(Truth)
  }
  
  Naive1 <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'naive1',tail(train$y,forecastPeriodLen),rep(NA,max(forecastPeriodLen-nrow(train),0)))
  if ((nrow(train)>(13+forecastPeriodLen))){
    Naive2 <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'naive2',train$y[(nrow(train)-13-forecastPeriodLen+1):(nrow(train)-13)])
  } else{
    Naive2 <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'naive2',rep(NA,forecastPeriodLen))
  }
  ...
  return(rbind(Truth,Naive1,Naive2,TBATS_fcast,ARIMA_fcast,Prophet_fcast,lm_fcast,RF_fcast,Xgboost_fcast,ets_fcast,rw_fcast,rwd_fcast,wn_fcast,nn_fcast)) # stlar_fcast does not work for the first CFG!
}
```
...
# All mine with HDD data from IRIS
...
HDD_Weekly_MRP$MRPVersion <- 'FY19W06'
rm(HDD_Weekly_MRP_Global,HDD_Weekly_MRP_Rgn)

# join
Fcast_Yao %>% full_join(HDD_Weekly_MRP) -> CFG_fcast.joined
...
# To visualize more historical data
...
CFG_fcast$Ensemble <- rowMeans(select(CFG_fcast,ARIMA,Xgboost), na.rm = TRUE)
...
LastWk <- tail(unique(HDD_Weekly_Global$Fiscal_Wk), n=1)
...
CFG_fcast <- filter(CFG_fcast,Fiscal_Wk>="FY19W07" & Fiscal_Wk<=LastWk) %>%
  ...
  mutate(err_MRP = abs(Actual - MRP_Fcast),
         err_Prophet = abs(Actual - Prophet),
         err_ARIMA = abs(Actual - ARIMA),
         err_TBATS = abs(Actual - TBATS),
         err_lm = abs(Actual - lm),
         err_RF = abs(Actual - RF),
         err_Xgboost = abs(Actual - Xgboost),
         err_ets = abs(Actual - ets),
         err_nn = abs(Actual - nn),
         err_rw = abs(Actual - rw),
         err_rwd = abs(Actual - rwd),
         err_wn = abs(Actual - wn),
         err_naive1 = abs(Actual - naive1),
         err_naive2 = abs(Actual - naive2),
         err_Ensemble = abs(Actual - Ensemble),
         # calculate absolute percent error
         ape_MRP = abs((Actual - MRP_Fcast)/Actual * 100),
         ape_Prophet = abs((Actual - Prophet)/Actual * 100),
         ape_ARIMA = abs((Actual - ARIMA)/Actual * 100),
         ape_TBATS = abs((Actual - TBATS)/Actual * 100),
         ape_lm = abs((Actual - lm)/Actual * 100),
         ape_RF = abs((Actual - RF)/Actual * 100),
         ape_Xgboost = abs((Actual - Xgboost)/Actual * 100),
         ape_ets = abs((Actual - ets)/Actual * 100),
         ape_nn = abs((Actual - nn)/Actual * 100),
         ape_rw = abs((Actual - rw)/Actual * 100),
         ape_rwd = abs((Actual - rwd)/Actual * 100),
         ape_wn = abs((Actual - wn)/Actual * 100),
         ape_naive1 = abs((Actual - naive1)/Actual * 100),
         ape_naive2 = abs((Actual - naive2)/Actual * 100),
         ape_Ensemble = abs((Actual - Ensemble)/Actual * 100))
...
results <- foreach(i_CFG=1:len_CFG, .combine=rbind, .packages=c('tidyverse')) %dopar% {
  ...
  for (i_Region in 1:len_Region){
    ...
    totalHDD <- sum(temp_data$Actual, na.rm = TRUE)
    w <- temp_data$Actual/totalHDD
    
    # MRP
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_MRP <- weighted.mean(temp_data$ape_MRP,temp_data$Actual, na.rm = TRUE)
    ...
    Aggregated_weighted_APE_MRP <- sum(w*temp_data$err_MRP, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    ...
    # Prophet
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_Prophet <- weighted.mean(temp_data$ape_Prophet,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_Prophet <- sum(w*temp_data$err_Prophet, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    if (all(is.na(temp_data$ARIMA))) {
      ...
      # Metric 1: Weighted MAPE by Week / WAPE
      mape_wk_ARIMA <- weighted.mean(temp_data$ape_ARIMA,temp_data$Actual, na.rm = TRUE)
      # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
      ...
      # Metric 2: Aggregated Weighted APE
      Aggregated_weighted_APE_ARIMA <- sum(w*temp_data$err_ARIMA, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
      # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # TBATS
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_TBATS <- weighted.mean(temp_data$ape_TBATS,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_TBATS <- sum(w*temp_data$err_TBATS, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    #  lm
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_lm <- weighted.mean(temp_data$ape_lm,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_lm <- sum(w*temp_data$err_lm, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    #  RF
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_RF <- weighted.mean(temp_data$ape_RF,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_RF <- sum(w*temp_data$err_RF, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # Xgboost
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_Xgboost <- weighted.mean(temp_data$ape_Xgboost,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_Xgboost <- sum(w*temp_data$err_Xgboost, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    # ets
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_ets <- weighted.mean(temp_data$ape_ets,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_ets <- sum(w*temp_data$err_ets, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_ets <- mean(abs(temp_data$err_ets/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_ets <- abs(sum(temp_data$err_ets, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    ...
    # nn
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_nn <- weighted.mean(temp_data$ape_nn,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_nn <- sum(w*temp_data$err_nn, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_nn <- mean(abs(temp_data$err_nn/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_nn <- abs(sum(temp_data$err_nn, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    ...
    # rw
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_rw <- weighted.mean(temp_data$ape_rw,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_rw <- sum(w*temp_data$err_rw, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_rw <- mean(abs(temp_data$err_rw/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_rw <- abs(sum(temp_data$err_rw, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    ...
    # rwd
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_rwd <- weighted.mean(temp_data$ape_rwd,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_rwd <- sum(w*temp_data$err_rwd, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_rwd <- mean(abs(temp_data$err_rwd/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_rwd <- abs(sum(temp_data$err_rwd, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    ...
    # wn
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_wn <- weighted.mean(temp_data$ape_wn,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_wn <- sum(w*temp_data$err_wn, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_wn <- mean(abs(temp_data$err_wn/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_wn <- abs(sum(temp_data$err_wn, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    ...
    # naive1
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_naive1 <- weighted.mean(temp_data$ape_naive1,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    WAPE_naive1 <- sum(temp_data$err_naive1, na.rm = TRUE)/totalHDD*100
    # Metric 1.3: Aggregated APE during the forecast period # The same as 1 and 1.2? Yes!
    APE_fcastPeriod_naive1 <- abs(sum(temp_data$naive1, na.rm = TRUE)-totalHDD)/totalHDD * 100
    
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_naive1 <- sum(w*temp_data$err_naive1, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_naive1 <- mean(abs(temp_data$err_naive1/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_naive1 <- abs(sum(temp_data$err_naive1, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    RMSE_wk_naive1 <- RMSE(temp_data$err_naive1)
    # Metric 6: Aggregatred RMSE/MAE per week
    AAE_fcastPeriod_per_week_naive1 <- abs(sum(temp_data$naive1, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_naive1 <- mean(temp_data$err_naive1, na.rm = TRUE)
    
    naive1 <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'naive1',mape_wk_naive1,WAPE_naive1,APE_fcastPeriod_naive1,Aggregated_weighted_APE_naive1,MASE_wk_naive1,AASE_fcastPeriod_naive1,RMSE_wk_naive1,AAE_fcastPeriod_per_week_naive1,MAE_wk_naive1)
    
    # naive2
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_naive2 <- weighted.mean(temp_data$ape_naive2,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    WAPE_naive2 <- sum(temp_data$err_naive2, na.rm = TRUE)/totalHDD*100
    # Metric 1.3: Aggregated APE during the forecast period # The same as 1 and 1.2? Yes!
    APE_fcastPeriod_naive2 <- abs(sum(temp_data$naive2, na.rm = TRUE)-totalHDD)/totalHDD * 100
    
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_naive2 <- sum(w*temp_data$err_naive2, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    MASE_wk_naive2 <- mean(abs(temp_data$err_naive2/temp_data$err_MRP), na.rm = TRUE)
    # Metric 4 Aggregated Absolute Scaled Error
    AASE_fcastPeriod_naive2 <- abs(sum(temp_data$err_naive2, na.rm = TRUE)/sum(temp_data$err_MRP, na.rm = TRUE))
    # Metric 5: Mean RMSE by Week
    RMSE_wk_naive2 <- RMSE(temp_data$err_naive2)
    # Metric 6: Aggregatred RMSE/MAE per week
    AAE_fcastPeriod_per_week_naive2 <- abs(sum(temp_data$naive2, na.rm = TRUE)-totalHDD)/nrow(temp_data)
    # Metric 6.2: MAE # The same as 6
    MAE_wk_naive2 <- mean(temp_data$err_naive2, na.rm = TRUE)
    
    naive2 <- c(CFGgroups2[i_CFG],Region.groups[i_Region],'naive2',mape_wk_naive2,WAPE_naive2,APE_fcastPeriod_naive2,Aggregated_weighted_APE_naive2,MASE_wk_naive2,AASE_fcastPeriod_naive2,RMSE_wk_naive2,AAE_fcastPeriod_per_week_naive2,MAE_wk_naive2)
    
    # Ensemble
    # Metric 1: Weighted MAPE by Week / WAPE
    mape_wk_Ensemble <- weighted.mean(temp_data$ape_Ensemble,temp_data$Actual, na.rm = TRUE)
    # Metric 1.2: WAPE # Is this the same as my Weighted MAPE by Week? Yep!
    ...
    # Metric 2: Aggregated Weighted APE
    Aggregated_weighted_APE_Ensemble <- sum(w*temp_data$err_Ensemble, na.rm = TRUE)/sum(w*temp_data$Actual, na.rm = TRUE)*100
    # Metric 3: MASE: Mean Absolute Scaled Error
    ...
    OneRow <- rbind(MRP,Prophet,ARIMA,TBATS,lm,RF,Xgboost,ets,nn,rw,rwd,wn,naive1,naive2,Ensemble)
...
path <- "~/Yao_Rdata/EvaluationResults_FY19W06.feather"
write_feather(results, path)
write_excel_csv(results.weight,"~/Yao_Rdata/EvaluationResults_FY19W06.csv")
```
