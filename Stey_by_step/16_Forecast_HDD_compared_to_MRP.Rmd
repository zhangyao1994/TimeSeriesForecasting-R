...
myforecast <- function(temp_data){
  ...
  if ((nrow(train)>=(13+forecastPeriodLen))){
    Naive2 <- c(CFGgroups[i_CFG],Regions[i_Region],MRP_Version,'naive2',train$y[(nrow(train)-13-forecastPeriodLen):(nrow(train)-13)])
  } else{
...
results <- foreach(i_CFG=1:len_CFG, .combine=rbind, .packages=c('tidyverse')) %dopar% {
  ...
  for (i_Region in 1:len_Region){
    ...
    totalHDD <- sum(temp_data$HDD_QTY, na.rm = TRUE)
    ...
    OneRow <- rbind(MRP,Prophet,ARIMA,TBATS,lm,RF,Xgboost,ets,nn,rw,rwd,wn,naive,Ensemble)
