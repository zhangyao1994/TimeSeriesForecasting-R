...
myforecast <- function(temp_data){
  ...
  Xgboost_fcast <- c(CFGgroups[i_CFG],'timetk_Xgboost',rep(NA,4),preds,rep(NA,28-4-11))
...
Comb_fcast[,3:30] <- lapply(Comb_fcast[,3:30], function(x) as.numeric(as.character(x)))

num_CFG_valid <- nrow(Comb_fcast)/6 # The total number of CFGs were forecasted.

Comb_fcast %>% gather(key='Fiscal_Wk',value='HDD_QTY',FY19W03:FY19W30) -> All_fcast
