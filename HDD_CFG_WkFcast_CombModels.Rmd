...
myforecast <- function(temp_data){
  ...
  return(rbind(TBATS_fcast,ARIMA_fcast,Prophet_fcast))
}
```
...
All_fcast <- foreach(i_CFG=1:len, .combine=rbind, .packages=c('tidyverse','forecast','Rcpp','ggthemes','lubridate','tidyquant','timetk','prophet')) %dopar% {
  # For each CFG
...
num_CFG_valid <- nrow(na.omit(All_fcast))/3 # The total number of CFGs were forecasted.

All_fcast %>% gather(key='Fiscal_Wk',value='HDD_QTY',FY19W03:FY19W30) -> All_fcast
saveRDS(All_fcast, 'All_fcast.rds') # my_data <- readRDS(file)
```
