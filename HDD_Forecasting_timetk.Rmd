...
# Daiy data
hdd_qty %>% group_by(Fiscal_Date) %>%
  summarise(Fiscal_Date_QTY=sum(PART_QTY)) -> HDD_Daily
```
...
HDD_Daily <- HDD_Daily %>% select(date,Fiscal_Date_QTY)

# Visualize data and training/testing regions
HDD_Daily %>%
    ggplot(aes(x = date, y = Fiscal_Date_QTY)) +
    geom_point(alpha = 0.5, color = palette_light()[[1]]) +
    labs(title = "HDD Daily Demand: Daily Scale", x = "Fiscal Date",y='Daily Demand') +
    theme_tq() +
    annotate("text", x = ymd("2017-03-20"), y = 100000,
             color = palette_light()[[1]], label = "Train Region") +
    annotate("text", x = ymd("2018-03-10"), y = 100000,
...
### Modeling
Start with the training set, which has the "date" and "Fiscal_date_QTY" columns.
...
fit_lm <- lm(Fiscal_Date_QTY ~ ., data = train_augmented)
```
...
*It is random but the residuals might be too large.*
...
pred_test <- test %>%
    ...
    mutate(.resid = Fiscal_Date_QTY - yhat)
head(pred_test)
```

```{r Results Visualization}
ggplot(aes(x = date), data = HDD_Daily) +
    labs(title = "HDD Daily Demand: Daily Scale", x = "Fiscal Date",y='Daily Demand') +
    theme_tq() +
    annotate("text", x = ymd("2017-03-20"), y = 100000,
             color = palette_light()[[1]], label = "Train Region") +
    ...
    geom_rect(xmin = as.numeric(ymd("2018-01-01")),
              ...
              ymin = 0, ymax = 120000,
              fill = palette_light()[[4]], alpha = 0.01) +
    geom_point(aes(x = date, y = Fiscal_Date_QTY), data = train, alpha = 0.5, color = palette_light()[[1]]) +
    geom_point(aes(x = date, y = Fiscal_Date_QTY), data = pred_test, alpha = 0.5, color = palette_light()[[1]]) +
...
error_tbl <- pred_test %>%
  mutate(pct_err = .resid/Fiscal_Date_QTY * 100) %>%
  summarize(
...
Let's use our model to predict What are the expected future values for the next six months. The first step is to create the date sequence. Let's use tk_get_timeseries_summary() to review the summary of the dates from the original dataset, "HDD_Daily".
...
HDD_future <- data_future %>%
    select(date) %>%
    add_column(Fiscal_Date_QTY = pred_future)
```
...
HDD_Daily %>%
    ...
    annotate("text", x = ymd("2017-03-20"), y = 90000,
             color = palette_light()[[1]], label = "Train Region") +
    annotate("text", x = ymd("2018-03-10"), y = 100000,
             color = palette_light()[[1]], label = "Test Region") +
    annotate("text", x = ymd("2018-08-20"), y = 110000,
             color = palette_light()[[1]], label = "Forecast Region") +
    geom_point(alpha = 0.5, color = palette_light()[[1]]) +
    geom_point(aes(x = date, y = Fiscal_Date_QTY), data = HDD_future,
               alpha = 0.5, color = palette_light()[[2]]) +
    geom_smooth(aes(x = date, y = Fiscal_Date_QTY), data = HDD_future,
                method = 'loess') + 
    labs(title = "HDD Daily Demand: 6-Month Forecast", x = "Fiscal Date",y='Daily Demand') +
    theme_tq()
...
HDD_future <- HDD_future %>%
    mutate(
        lo.95 = Fiscal_Date_QTY - 1.96 * test_resid_sd,
        lo.80 = Fiscal_Date_QTY - 1.28 * test_resid_sd,
        hi.80 = Fiscal_Date_QTY + 1.28 * test_resid_sd,
        hi.95 = Fiscal_Date_QTY + 1.96 * test_resid_sd
        )
```
...
HDD_Daily %>%
    ...
    labs(title = "HDD Daily Demand: 6-Month Forecast with Prediction Intervals",x = "Fiscal Date",y='Daily Demand') +
    theme_tq()
```
