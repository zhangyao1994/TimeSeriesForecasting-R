...
Let me work on weekly demand here then.
...
# Daiy data
hdd_qty %>% group_by(Fiscal_Wk_End_Date) %>%
...
HDD_Weekly <- HDD_Weekly %>%
  filter(date>'2016-04-01' & date<'2018-05-25') %>% select(date,HDD_QTY)
...
HDD_Weekly %>%
    ...
    geom_line() +
    labs(title = "HDD Weekly Demand: Weekly Scale", x = "Fiscal Date",y='Weekly Demand') +
    theme_tq() +
...
Anyways, let me validate this Weekly model.
...
ggplot(aes(x = date), data = HDD_Weekly) +
    labs(title = "HDD Weekly Demand: Weekly Scale", x = "Fiscal Date",y='Weekly Demand') +
    theme_tq() +
    annotate("text", x = ymd("2017-03-20"), y = 220000,
             color = palette_light()[[1]], label = "Train Region") +
...
Let's use our model to predict What are the expected future values for the next six months. The first step is to create the date sequence. Let's use tk_get_timeseries_summary() to review the summary of the dates from the original dataset, "HDD_Weekly".
...
idx_future <- idx %>%
    tk_make_future_timeseries(n_future = 24)
```
...
HDD_Weekly %>%
    ...
    labs(title = "HDD Weekly Demand: 6-Month Forecast", x = "Fiscal Date",y='Weekly Demand') +
    theme_tq()
...
HDD_Weekly %>%
    ...
    labs(title = "HDD Weekly Demand: 6-Month Forecast with Prediction Intervals",x = "Fiscal Date",y='Weekly Demand') +
    theme_tq()
```
