...
p_week <- temp_data %>%
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = 'Overall HDD Weekly Sales FY17W01-FY19W15', x = "Fiscal Week", y = "Part Quantity") + 
...
p_week
# This weekly plot is too detalied to find obvious patterns.

# HDD Monthly Sales
unique(HDD_data$Fiscal_Mo)

temp_data <- HDD_data %>% group_by(Fiscal_Mo) %>%
...
p_month <- temp_data %>%
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = 'HDD Monthly Sales FY17M01-FY19M03', x = "Fiscal Month", y = "Part Quantity") + 
  theme_minimal(base_size = 18) + 
  scale_color_tableau('tableau10medium') + 
...
# HDD Quarterly Sales
unique(HDD_data$Fiscal_Qtr)

temp_data <- HDD_data %>% group_by(Fiscal_Qtr) %>%
...
p_quarter <- temp_data %>%
  ...
  labs(title = 'Overall HDD Quarterly Sales FY17Q1-FY19Q1', x = "Fiscal Quarter", y = "Part Quantity") + 
...
HDD_data %>% 
  ...
  filter(Fiscal_Qtr<'FY19Q2')->temp_data
...
p_Quarterly_Region <- temp_data %>%
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = 'Overall HDD Quarterly Sales by Region FY17Q1-FY19Q1', x = "Fiscal Quarter", y = "Part Quantity") + 
  theme_minimal(base_size = 14) + 
  #scale_color_tableau('tableau10medium') + 
  #scale_x_discrete(breaks = c('FY17Q2', 'FY18Q1', 'FY19Q1')) +
  theme(legend.position = 'bottom',plot.title = element_text(hjust = 0.5))+
...
p_Quarterly_Region_Pie <- temp_data %>%
  ...
  coord_polar("y") +
  labs(title = 'Overall HDD Quarterly Sales by Region FY17Q2-FY19Q1',fill="Region")
...
```{r customer analysis}
# Only by Customer
length(unique(HDD_data$Customer))

HDD_data %>%  filter(Fiscal_Qtr<'FY19Q2') %>%
  group_by(Customer) %>%
...
#   filter(Fiscal_Mo>'FY17M03' & Fiscal_Mo<'FY19M04') %>%
...
#   geom_point(size = 2) +
#   geom_line(size = 1.5) +
#   labs(title = 'HDD Monthly Sales Top 30 Customers FY17M04-FY19M03', x = "Fiscal Month", y = "Part Quantity") + 
#   #theme_minimal(base_size = 18) + 
#   scale_color_tableau('tableau10medium') + 
#   scale_x_discrete(breaks = c('FY17M05','FY18M02', 'FY19M01')) +
...
p_Quarterly_Customer_LOB <- data4plot %>% 
  ...
  labs(title = 'Overall HDD Quarterly Sales by Customers and LOB FY17Q1-FY19Q1', x = "Fiscal Quarter", y = "Part Quantity") + 
  theme_minimal(base_size = 14) + 
  scale_color_tableau('tableau10medium') + 
...
HDD_data %>% 
  ...
  summarise(LOB_DESC_Fiscal_Mo_QTY=sum(PART_QTY)) %>%
  filter(Fiscal_Mo<'FY19M04') -> temp_data
...
p_Monthly_LOB_DESC <- temp_data %>%
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = 'HDD Monthly Sales by LOB_DESC FY17M01-FY19M03', x = "Fiscal Month", y = "Part Quantity") + 
  theme_minimal(base_size = 18) + 
  scale_color_tableau('tableau10medium') + 
  scale_x_discrete(breaks = c('FY17M01','FY18M01', 'FY19M01')) +
  theme(legend.position = 'bottom',plot.title = element_text(hjust = 0.5))+
...
HDD_data %>% 
  ...
  summarise(LOB_DESC_Fiscal_Qtr_QTY=sum(PART_QTY)) %>%
  filter(Fiscal_Qtr<'FY19Q2')->temp_data
...
p_Quarterly_LOB_DESC <- temp_data %>%
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = 'Overall HDD Quarterly Sales by LOB FY17Q1-FY19Q1', x = "Fiscal Quarter", y = "Part Quantity",colour='LOB') + 
  theme_minimal(base_size = 14) + 
  scale_color_tableau('tableau10medium') + 
  #scale_x_discrete(breaks = c('FY17Q2', 'FY18Q1', 'FY19Q1')) +
  theme(legend.position = 'bottom',plot.title = element_text(hjust = 0.5))+
...
HDD_data %>% 
  ...
  summarise(CFG_Fiscal_Qtr_QTY=sum(PART_QTY)) %>%
  filter(Fiscal_Qtr<'FY19Q2') -> temp_qtr

HDD_data %>% 
  group_by(CFG,Fiscal_Mo) %>%
  summarise(CFG_Fiscal_Mo_QTY=sum(PART_QTY)) %>%
  filter(Fiscal_Mo<'FY19M04') -> temp_mo

HDD_data %>% 
  group_by(CFG,Fiscal_Wk) %>%
  summarise(CFG_Fiscal_Wk_QTY=sum(PART_QTY)) %>%
  filter(Fiscal_Wk<'FY19W16') -> temp_wk

# temp_data <- temp_qtr

CFGgroups <- levels(factor(temp_data$CFG))
...
for (i in 1:len){
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = paste(CFGgroups[i],'HDD Weekly Sales FY17W01-FY19W15'), x = "Fiscal Week", y = "Part Quantity") + 
  theme_minimal(base_size = 14) + 
  scale_color_tableau('tableau10medium') + 
  scale_x_discrete(breaks = c('FY17W01', 'FY18W01', 'FY19W01')) +
  theme(legend.position = 'none',plot.title = element_text(hjust = 0.5)) +
  ...
  geom_point(size = 2) +
  geom_line(size = 1.5) +
  labs(title = paste(CFGgroups[i],'HDD Monthly Sales FY17M01-FY19M03'), x = "Fiscal Month", y = "Part Quantity") + 
  theme_minimal(base_size = 14) + 
  scale_color_tableau('tableau10medium') + 
  scale_x_discrete(breaks = c('FY17M01','FY18M01', 'FY19M01')) +
  theme(legend.position = 'none',plot.title = element_text(hjust = 0.5)) +
  ...
  geom_line(size = 1.5) +
  #geom_bar(stat = 'identity') +
  labs(title = paste(CFGgroups[i],'HDD Quarterly Sales FY17Q1-FY19Q1'), x = "Fiscal Quarter", y = "Part Quantity") + 
  theme_minimal(base_size = 14) + 
  scale_color_tableau('tableau10medium') + 
  theme(legend.position = 'none',plot.title = element_text(hjust = 0.5))+
  scale_y_continuous(label=comma) + expand_limits(y = 0)
