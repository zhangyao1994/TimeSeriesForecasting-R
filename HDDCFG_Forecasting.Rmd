...
```{r load the data}
HDD_data0 <- read.csv("~/HDD_FY16_FY19M03_Transformed_filledAllFiscalQuarter.csv") %>% filter(Fiscal_Yr %in% c('FY17','FY18','FY19'))
```

```{r Extract Different CFGs}
CFG_levels <- levels(HDD_data0$CFG)

HDD_data <- filter(HDD_data0,CFG == CFG_levels[2])
