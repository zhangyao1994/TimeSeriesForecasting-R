...
6/11/2018 Forecast F19W07-W15 and then compare my forecasting TBATS_fcast to MRP.
...
TBATS_fcast <- foreach(i_CFG=1:len, .combine=rbind, .packages=c('tidyverse','forecast','Rcpp','ggthemes','lubridate','tidyquant')) %dopar% {
  # For each CFG
...
dimnames(TBATS_fcast)[[2]] <- c('CFG',as.character(HDD_Weekly$Fiscal_Wk[112:120]))
TBATS_fcast <- data.frame(TBATS_fcast)
TBATS_fcast[,2:10] <- lapply(TBATS_fcast[,2:10], function(x) as.numeric(as.character(x)))

num_CFG_valid <- nrow(na.omit(TBATS_fcast)) # The total number of CFGs were forecasted.

TBATS_fcast %>% gather(key='date',value='TBATS_fcast',FY19W07:FY19W15) -> TBATS_fcast
